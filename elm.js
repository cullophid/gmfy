...
Elm.Test.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var route = F3(function (condition,content,location) {
      var _p0 = condition(location);
      if (_p0 === true) {
            return content;
         } else {
            return $Html.text("");
   ...
   var update = F2(function (action,model) {
      var _p1 = action;
      if (_p1.ctor === "Location") {
            return _U.update(model,{location: _p1._0});
         } else {
            return model;
         }
   });
   var view = function (model) {
      var _p2 = model;
      var location = _p2.location;
      return A2($Html.div,
      _U.list([]),
      _U.list([A3(route,$Regex.contains($Regex.regex("route1")),$Html.text("route 1"),location)
              ,A3(route,$Regex.contains($Regex.regex("route2")),$Html.text("route 2"),location)]));
   };
   var emptyModel = {location: ""};
   var Location = function (a) {    return {ctor: "Location",_0: a};};
   var model = A3($Signal.foldp,update,emptyModel,A2($Signal.map,Location,$History.hash));
